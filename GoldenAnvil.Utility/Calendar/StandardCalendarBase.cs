...
public abstract class StandardCalendarBase : ICalendar
{
	protected StandardCalendarBase(StandardCalendarConfig config)
	{
	...
	public TimePoint CreateTimePoint(int year, int month, int day, int hour, int minute, int second)
	{
		...
		return new(secondsForDays + secondsForHours + secondsForMinutes + second);
	}
	...
	public TimeOffset CreateTimeOffset(int days, int hours, int minutes, int seconds)
	{
		...
		return new TimeOffset(secondsForDays + secondsForHours + secondsForMinutes + seconds);
	}

	public TimePoint GetStartOfDay(TimePoint time) =>
		new(m_secondsInDay * (time.Seconds / m_secondsInDay));

	public string FormatTime(TimePoint point, TimeFormat format)
	{
		(var years, var months, var days, var hours, var minutes, var seconds) = SplitSeconds(point.Seconds);
	...
	{
		if (offset.SecondOffset < 0)
			throw new ArgumentException($"{nameof(offset)} must be positive");

		(var startYears, var startMonths, var startDays, var startHours, var startMinutes, var startSeconds) = SplitSeconds(point.Seconds);
		(var endYears, var endMonths, var endDays, var endHours, var endMinutes, var endSeconds) = SplitSeconds((point + offset).Seconds);
