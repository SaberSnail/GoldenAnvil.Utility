...
public class CryptoRandom : Random
{
	...
	public override void NextBytes(byte[] buffer)
	{
		...
		lock (this)
		{
			if (IsRandomPoolEnabled && m_buffer == null)
				InitBuffer();

			// Can we fit the requested number of bytes in the buffer?
			if (IsRandomPoolEnabled && m_buffer.Length <= buffer.Length)
			{
				int count = buffer.Length;

				EnsureRandomBuffer(count);

				Buffer.BlockCopy(m_buffer, m_bufferPosition, buffer, 0, count);
	...
	private void InitBuffer()
	{
		...
		{
			if (m_buffer == null || m_buffer.Length != 512)
				m_buffer = new byte[512];
		}
		else
		{
			if (m_buffer == null || m_buffer.Length != 4)
				m_buffer = new byte[4];
		}

		m_rng.GetBytes(m_buffer);
		m_bufferPosition = 0;
	}
	...
	private uint GetRandomUInt32()
	{
		lock (this)
		{
			...
			uint rand = BitConverter.ToUInt32(m_buffer, m_bufferPosition);
